...
if (checkCompatibility()) {
    ...
    connectBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            //-- Configurar para trabajar con la UART
            await ftdi.uart_init(device);
            log("Interfaz B configurada como UART a 115200 baudios", "system");
...
async function handleUpload() {
    ...
    try {
        ...
        if (verifyCheckbox.checked) {
            await verifyProgramming(data);
        }

        fileStatus.textContent = "¡Proceso terminado con éxito!";

    } catch (err) {
        log("❌ FALLO: " + err.message, "error");
        progressBar.style.background = "#ef4444"; // Rojo en caso de error
        fileStatus.textContent = "Error en el proceso.";
    } finally {
        //-- Quitar el Reset de la FPGA (opcional)
        await ftdi.FPGA_reset_deassert(device);
        uploadBtn.disabled = false;
        // Restaurar color original de la barra tras un tiempo
        setTimeout(() => { progressBar.style.background = "#3b82f6"; }, 3000);
    }
}
...
async function verifyProgramming(originalData) {
    const totalSize = originalData.length;
    log(`Iniciando verificación de ${totalSize} bytes...`, "system");
    
    progressBar.style.width = '0%';
    progressBar.style.background = '#eab308'; // Color amarillo para diferenciar de grabación
    fileStatus.textContent = "Verificando integridad...";

    //-- Tamaño del bloque a leer cada vez
    let size = 64;

    //-- Buffer donde almacenar los bytes leidos de la flash
    let buf_flash = new Uint8Array(totalSize);
    ...
    if (!array_equal(buf_flash, originalData, offset)) {
