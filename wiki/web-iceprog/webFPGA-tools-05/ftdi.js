...
const INTERFACE_B   = 2;  //-- Interfaz B (UART)
...
//-- COMANDOS del FTDI para la UART
const SET_BAUD_RATE = 0x03;     //-- Configurar los baudios
const SET_LINE_PROPERTY = 0x04; //-- Configurar modo transmision

//------ Constantes para los comandos
const B115200 = 0x001A;  // Divisor para 115200
//-- (8 bits datos, sin paridad, 1 bit stop) 
const CONF_8N1 = 0x0008; //-- Valor para 8-N-1 
...
export async function initialize(device) {
    ...
    //-- Reclamar la interfaz 1 (B). Es la que usaremos
    //-- para la UART
...
//----------------------------------------------------
//-- Configurar el FTDI para trabajar con la UART
//-- Interfaz B
//----------------------------------------------------
export async function uart_init(device)
{
    //--1. Inicializacion y configuración del interfaz B
    await sio_resetB(device);

    // 2. Configurar Baudios (115200 baudios)
    await uart_set_baud_rate(device);

    // 3. Configurar trama: 8 bits de datos, 1 stop bit, sin paridad
    await uart_set_line_property(device);
}
...
export async function sio_resetB(device) {
    await device.controlTransferOut({
        requestType: 'vendor',
        recipient: 'device',
        request: SIO_RESET_REQUEST, 
...
export async function uart_set_baud_rate(device)
{
    // 2. Configurar Baudios (Cálculo para 115200 baudios)
    await device.controlTransferOut({
        requestType: 'vendor',
        recipient: 'device',
        request: SET_BAUD_RATE, 
        value: B115200,
        index: INTERFACE_B
    });
}

export async function uart_set_line_property(device)
{
    // 3. Configurar trama: 8 bits de datos, 1 stop bit, sin paridad
    await device.controlTransferOut({
        requestType: 'vendor',
        recipient: 'device',
        request: SET_LINE_PROPERTY,
        value: CONF_8N1,
        index: INTERFACE_B
    });
}
